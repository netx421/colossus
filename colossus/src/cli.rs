...
use colossus_config::SerdeReplace;
use clap::{ArgAction, Args, Parser, Subcommand, ValueHint};
use log::{LevelFilter, error};
use serde::{Deserialize, Serialize};
use toml::Value;

use colossus_terminal::tty::Options as PtyOptions;
...
pub struct Options {
    ...
    /// $XDG_CONFIG_HOME/colossus/colossus.toml].
    #[cfg(not(any(target_os = "macos", windows)))]
    #[clap(long, value_hint = ValueHint::FilePath)]
    pub config_file: Option<PathBuf>,

    /// Specify alternative configuration file [default: %APPDATA%\colossus\colossus.toml].
    #[cfg(windows)]
    #[clap(long, value_hint = ValueHint::FilePath)]
    pub config_file: Option<PathBuf>,

    /// Specify alternative configuration file [default: $HOME/.config/colossus/colossus.toml].
    #[cfg(target_os = "macos")]
    #[clap(long, value_hint = ValueHint::FilePath)]
    pub config_file: Option<PathBuf>,
...
mod tests {
    ...
    fn completions() {
        ...
        for (shell, file) in &[
            (Shell::Bash, "colossus.bash"),
            (Shell::Fish, "colossus.fish"),
            (Shell::Zsh, "_colossus"),
        ] {
            let mut generated = Vec::new();
            clap_complete::generate(*shell, &mut clap, "colossus", &mut generated);
        ...
        //
        // let mut file = File::create("../extra/completions/colossus.bash").unwrap();
        // clap_complete::generate(Shell::Bash, &mut clap, "colossus", &mut file);
        // let mut file = File::create("../extra/completions/colossus.fish").unwrap();
        // clap_complete::generate(Shell::Fish, &mut clap, "colossus", &mut file);
        // let mut file = File::create("../extra/completions/_colossus").unwrap();
        // clap_complete::generate(Shell::Zsh, &mut clap, "colossus", &mut file);
    }
}
